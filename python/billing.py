...
from common import is_expired_pst_value, now_pst_naive, parse_duration_days
...
def register_routes(app):
    @app.route("/python/billing.py", methods=["POST"])
    def billing_php():
        conn = None
        try:
            conn = get_connection()
                ...
                if "otp" in form:
                    try:
                        ...
                        if otp_row:
                            ...
                            conn.begin()
                            with conn.cursor() as cur:
                                cur.execute("UPDATE otp SET is_used = TRUE WHERE id = %s", (otp_row.get("id"),))
                            ...
                            if not plan_id:
                                with conn.cursor() as cur:
                                    cur.execute(
                                        "INSERT INTO plans (goal, diet, mealtype, duration_days, price) VALUES (%s, %s, %s, %s, %s) RETURNING planid",
                                        (plan_goal, plan_diet, plan_mealtype, duration_days, plan_price),
                                    )
                                    result = cur.fetchone()
                                ...
                                for meal_name in meal_names:
                                    ...
                                    if not meal_id:
                                        with conn.cursor() as cur:
                                            cur.execute("INSERT INTO meals (meal_name) VALUES (%s) RETURNING mealid", (meal_name,))
                                            result = cur.fetchone()
                                            meal_id = result[0] if result else None

                                    with conn.cursor() as cur:
                                        cur.execute(
                                            "INSERT INTO plan_meals (planid, mealid) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                                            (plan_id, meal_id),
                                        )
                            ...
                            with conn.cursor() as cur:
                                cur.execute(
                                    "INSERT INTO subscriptions (userid, planid, start_date, end_date, status) VALUES (%s, %s, %s, %s, %s) RETURNING subscriptionid",
